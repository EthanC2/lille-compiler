...
static const int IEEE_754_EXPONENT_MIN = -126;
static const int IEEE_754_EXPONENT_MAX = 127;
...
{
    // 1. Reset previous string value; initialize loop sentinal
    this->current_symbol = new symbol(symbol::nul);
    this->current_string_value.clear();
    bool end_of_string {false};

    if (this->debugging)
    {
        std::cout << "[FUNCTION] entering " << __FUNCTION__ << '\n';
    }

    // 2. Read characters until a single '"' is encountered
    while (not end_of_string)
    {
        // 2A. Read the next character (discarding the initial '"' character)
        this->get_char();
        
        // EDGE CASE: reached end of line before reaching end of string
        if (next_char == this->end_marker)
        {
            // error code 60: string must be terminated before end of line
            this->error->flag(this->current_line_number, this->current_pos_on_line, 60);
            return;
        }

        // 2C. Special case: if the character is a '"', check if "" or single "
        if (this->next_char == '"')
        {
            // 2D. If the following character is '"', transform double "" into single " char
            if (this->following_char() == '"')
            {
                this->current_string_value +=  '"';
                this->get_char(); /* consume second '"' character */
            }
            else // 2E. If single '"', mark the end of the string
            {
                end_of_string = true;
            }
        }
        else // 2F. If a normal character, append its value to the string
        {
            this->current_string_value += this->next_char;
        }
    }

    // 3. Set the current symbol to indicate a successful parse. String value
    //    is contained in this->current_string_value
    if (this->current_string_value.length() > 0)
    {
        this->current_symbol = new symbol(symbol::strng);
    }
    else
    {
        this->error->flag(this->current_line_number, this->current_pos_on_line, 1);
    }

    if (this->debugging)
    {
        std::cout << "[FUNCTION] exiting " << __FUNCTION__ << '\n';
    }
}
...
void scanner::scan_digit()
{
    ...
    // 0. Temporarily set current symbol to symbol::nul in case of error
    this->current_symbol = new symbol(symbol::nul);
    if (this->debugging)
    {
        std::cout << "0. Parsing digit starting with " << this->next_char << '\n';
    }

    // 1. Parse integer or characteristic of real
    while (isdigit(this->next_char))
    {
        if (debugging)
        {
            std::cout << "1A. Parsed digit: " << this->next_char << '\n';
        }

        number += this->next_char;
        this->get_char();
    }

    if (this->debugging)
    {
        std::cout << "1B. Parsed integer: " << number << '\n';
    }

    // 2. Parse separator and mantissa if real number
    if (this->next_char == '.' and this->following_char() != '.') /* EDGE CASE: range symbol */
    {
        if (this->debugging)
        {
            std::cout << "2A. Found '.' after integer. Parsing Real.\n";
        }
        ...
        if (this->debugging)
        {
            std::cout << "2B. NUMBER = " << number << '\n';
        }

        if (not isdigit(this->next_char))
        {
            this->error->flag(this->current_line_number, this->current_pos_on_line, 63);
            return;
        }
        ...
        while (isdigit(this->next_char))
        {
            if (this->debugging)
            {
                std::cout << "2C. Parsed digit: " << this->next_char << '\n';
            }
        ...
        if (this->debugging)
        {
            std::cout << "2D. NUMBER = " << number << '\n';
        }
    }

    // 3. Parse exponent
    if (toupper(this->next_char) == 'E')
    {
        if (this->debugging) 
        {
            std::cout << "3A. Parsed 'E' after integer " << number << '\n';
        }
        ...
        if (this->debugging)
        {
            std::cout << "3A. NUMBER = " << number << '\n';
        }

        if (this->following_char() == '+' or this->following_char() == '-')
        {
            this->get_char();

            if (debugging)
            {
                std::cout << "3B. Parsed '+'/'-' after 'E'. Sign = " << this->next_char;
            }

            if (is_integer and this->next_char == '-')
            {
                this->error->flag(this->current_line_number, this->current_pos_on_line, 67);
                return;
            }

            number += this->next_char;
            exponent += this->next_char;

            if (this->debugging)
            {
                std::cout << "3B. NUMBER = " << number << '\n';
                std::cout << "3B. EXPONENT = " << exponent << '\n';
            }
        }

        if (not isdigit(this->following_char()))
        {
            this->error->flag(this->current_line_number, this->current_pos_on_line, 64);
            return;
        }
        else
        {
            ...
            while (isdigit(this->next_char))
            {
                if (debugging)
                {
                    std::cout << "3C. Parsed digit '" << this->next_char << "'\n";
                }
            ...
            if (this->debugging)
            {
                std::cout << "3D. Read digits after exponent. NUMBER = " << number << '\n';
                std::cout << "3D. Read digits after exponent. EXPONENT = " << number << '\n';
            }
        }
    }
    else if (debugging)
    {
        std::cout << "3D. No 'E' after number. Found '" << this->next_char << "'\n";
    }
    ...
    if (is_integer)
    {
        if (debugging)
        {
            std::cout << "4A. Assigning '" << number << "' to current integer value\n";
        }

        try
        {
            float value = std::stof(number);
            if (value < std::numeric_limits<int>::min() or value > std::numeric_limits<int>::max())
            {
                throw std::out_of_range{"constant not in range of a integer"};
            }

            this->current_integer_value = value;
            this->current_symbol = new symbol(symbol::integer);
        }
        catch(const std::exception&)
        {
            this->error->flag(this->current_line_number, this->current_pos_on_line, 68);
            return;
        }

        if (debugging)
        {
            std::cout << "4B. current_integer_value = " << current_integer_value << '\n';
        }
    }
    else
    {
        if (debugging)
        {
            std::cout << "4A. Assigning '" << number << "' to current real value\n";
        }
        ...
        try
        {
            if (exponent.length() > 0)
            {
                try
                {
                    int iexponent = std::stoi(exponent);

                    if (iexponent < IEEE_754_EXPONENT_MIN or iexponent > IEEE_754_EXPONENT_MAX)
                    {
                        throw std::out_of_range {"exponent not representable by IEEE 754 single-precision float (range: [-126,127])"};
                    }
                }
                catch(const std::exception&)
                {
                    this->error->flag(this->current_line_number, this->current_pos_on_line, 65);
                    return;
                }
            }

            this->current_real_value = std::stof(number);
            this->current_symbol = new symbol(symbol::real_num);
        }
        catch(const std::exception&)
        {
            this->error->flag(this->current_line_number, this->current_pos_on_line, 66);
            return;
        }

        if (debugging)
        {
            std::cout << "4B. current_real_value = " << current_real_value << '\n';
        }
    }
